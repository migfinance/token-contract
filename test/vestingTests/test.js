...
describe("LinearVesting", function () {
  ...
  beforeEach(async () => {
    ...
    linearVesting = await LinearVesting.deploy(migFinance.address, Math.floor(Date. now() / 1000)+10, Math.floor(Date. now() / 1000)+100);
  ...
  it("should create Beneficiary schedule", async () => {
    ...
    created.wait();
    expect(await linearVesting.vestedAmount("0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266")).to.equal("70000000000");

  });

  it("should draw dow Beneficiary amount", async () => {
